...
When attempting to upload files (especially batch uploads with multiple files or ZIP files), you experienced:
...
✅ ZIP file uploads are now supported  
...
3. **ZIP File Upload**
   - Go to the Dashboard
   - Select "Multiple Entries" mode
   - Choose a ZIP file containing multiple entry forms
   - Click "Process Entries"
   - **Expected**: ZIP file should be extracted and all files processed
   - **Expected**: All entries from the ZIP should appear in Review page

4. **Mixed Upload (ZIP + Individual Files)**
   - Go to the Dashboard
   - Select "Multiple Entries" mode
   - Choose both a ZIP file AND individual files
   - Click "Process Entries"
   - **Expected**: All files (from ZIP and individual) should process successfully
...
- What type of upload you were attempting (single, batch, ZIP)
...
- **File Types**: The system accepts PDF, PNG, and JPG files. ZIP files containing these formats are also supported.
